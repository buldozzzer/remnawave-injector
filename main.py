...
def should_skip_subscription_modification(flow: http.HTTPFlow) -> bool:
    ...
    for regex in EXCLUDE_UA_REGEXES:
        if regex.search(user_agent):
            logger.info(
                "[SUB] Модификация пропущена по regex",
    ...
    for excluded in EXCLUDE_UA_STRINGS:
        if excluded and excluded.lower() in user_agent.lower():
            logger.info(
                "[SUB] Модификация пропущена по строке", user_agent=user_agent[:100]
            )
            return True
...
def is_subscription_response(flow: http.HTTPFlow) -> bool:
    ...
    if any(
        ct in content_type
        for ct in ["text/plain", "application/json", "application/octet-stream"]
    ):
        return True
...
def modify_base64_subscription(content: bytes) -> bytes:
    ...
    try:
        ...
        if base64_config.get("enabled_filtering", False):
            if keep_keywords := [
                k.lower() for k in base64_config.get("keep_if_contains", [])
            ]:
                lines = [
                    line
                    for line in lines
                    if any(kw in line.lower() for kw in keep_keywords)
                ]

            if base64_config.get("enabled_removing", False):
                if remove_keywords := [
                    k.lower() for k in base64_config.get("remove_if_contains", [])
                ]:
                    lines = [
                        line
                        for line in lines
                        if not any(kw in line.lower() for kw in remove_keywords)
                    ]
...
def response(flow: http.HTTPFlow):
    ...
    logger.info(
        f"[← RESPONSE]", status_code=flow.response.status_code, url=flow.request.url
    )
...
if __name__ == "__main__":
    ...
    mitmdump(
        [
            ...
            str(LISTEN_PORT),
            "--mode",
            f"reverse:http://{BACKEND_HOST}:{BACKEND_PORT}",
            "--ssl-insecure",
            "--quiet",
            "--no-http2",
            "--set",
